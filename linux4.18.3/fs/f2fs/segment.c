...
void f2fs_balance_fs(struct f2fs_sb_info *sbi, bool need)
{
	...
	if (has_not_enough_free_nvm_secs(sbi->nsbi)) {
        mutex_lock(&sbi->nsbi->nvmgc_mutex);
		f2fs_nvm_gc(sbi, false, false, NULL_SEGNO);
...
static void new_curseg(struct f2fs_sb_info *sbi, int type, bool new_sec)
{
    ...
    if(type==CURSEG_HOT_DATA || type==CURSEG_HOT_NODE){
        //TODO:分配失败时相关处理
        nvm_allocate_segment(sbi,curseg->segno);
	}
    /*end*/
}
...
static void change_curseg(struct f2fs_sb_info *sbi, int type)
{
	...
	nvm_debug(NVM_INFO,"SSR:change_curseg happened");
