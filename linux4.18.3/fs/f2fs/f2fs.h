...
struct f2fs_io_info {
	...
	struct page *page;	/* page to be written。一个fio结构有一个待写的page */
	struct page *encrypted_page;	/* encrypted page ，加密的page*/
...
struct f2fs_bio_info {
	...
	struct bio *bio;		/* bios to merge,合并之后的bio */
	sector_t last_block_in_bio;	/* last block number, 合并之后的bio中的最后一个块号 */
...
struct f2fs_sb_info {
	...
	/* for bio operations */
	struct f2fs_bio_info *write_io[NR_PAGE_TYPE];	/* for write bios。页类型的数目NR_PAGE_TYPE = 3: Data, Node, Meta */
